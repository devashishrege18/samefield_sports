...
import { Award, Star, Users, TrendingUp, Lock, Heart, MessageCircle, Share2, Send, Zap, ChevronRight, ChevronLeft, Globe, Image as ImageIcon, Play, X, Pin, ExternalLink, RefreshCw } from 'lucide-react';
...
const Fandom = () => {
    ...
    const [igCanScrollLeft, setIgCanScrollLeft] = useState(false);
    const [igCanScrollRight, setIgCanScrollRight] = useState(true);
    const [twCanScrollLeft, setTwCanScrollLeft] = useState(false);
    const [twCanScrollRight, setTwCanScrollRight] = useState(true);
    ...
    const scrollInstagram = (direction) => {
        const container = igScrollRef.current;
        if (!container) return;
    ...
    const updateIgScrollState = () => {
        const container = igScrollRef.current;
        if (!container) return;
        setIgCanScrollLeft(container.scrollLeft > 10);
        setIgCanScrollRight(container.scrollLeft < container.scrollWidth - container.clientWidth - 10);
    };

    // Scroll functions for Twitter feed
    const scrollTwitter = (direction) => {
        const container = twScrollRef.current;
        if (!container) return;
    ...
    const updateTwScrollState = () => {
        const container = twScrollRef.current;
        if (!container) return;
        setTwCanScrollLeft(container.scrollLeft > 10);
        setTwCanScrollRight(container.scrollLeft < container.scrollWidth - container.clientWidth - 10);
    };
                    ...
                    {activeTab === 'media' && (
                        <div className="fade-in space-y-8">
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <div className="flex items-center gap-1">
                                        <button
                                            ...
                                            disabled={!igCanScrollLeft}
                                            className={`w-7 h-7 flex items-center justify-center rounded-full border transition-all ${igCanScrollLeft
                                                ? 'bg-surfaceHighlight border-white/20 text-white hover:bg-gradient-to-tr hover:from-yellow-500 hover:via-pink-500 hover:to-purple-600 hover:border-pink-500'
                                        ...
                                        <button
                                            ...
                                            disabled={!igCanScrollRight}
                                            className={`w-7 h-7 flex items-center justify-center rounded-full border transition-all ${igCanScrollRight
                                                ? 'bg-surfaceHighlight border-white/20 text-white hover:bg-gradient-to-tr hover:from-yellow-500 hover:via-pink-500 hover:to-purple-600 hover:border-pink-500'
                                                : 'bg-surface border-white/10 text-white/30 cursor-not-allowed'}`}
                                        >
                                ...
                                <div ref={igScrollRef} onScroll={updateIgScrollState} className="flex gap-4 overflow-x-auto pb-4" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <div className="flex items-center gap-1">
                                        <button
                                            onClick={() => scrollTwitter('left')}
                                            disabled={!twCanScrollLeft}
                                            className={`w-7 h-7 flex items-center justify-center rounded-full border transition-all ${twCanScrollLeft
                                                ? 'bg-surfaceHighlight border-white/20 text-white hover:bg-white hover:text-black hover:border-white'
                                                : 'bg-surface border-white/10 text-white/30 cursor-not-allowed'}`}
                                        >
                                            <ChevronLeft className="w-4 h-4" />
                                        </button>
                                        <button
                                            onClick={() => scrollTwitter('right')}
                                            disabled={!twCanScrollRight}
                                            className={`w-7 h-7 flex items-center justify-center rounded-full border transition-all ${twCanScrollRight
                                                ? 'bg-surfaceHighlight border-white/20 text-white hover:bg-white hover:text-black hover:border-white'
                                                : 'bg-surface border-white/10 text-white/30 cursor-not-allowed'}`}
                                        >
                                            <ChevronRight className="w-4 h-4" />
                                ...
                                <div ref={twScrollRef} onScroll={updateTwScrollState} className="flex gap-4 overflow-x-auto pb-4" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
